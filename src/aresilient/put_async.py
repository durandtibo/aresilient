r"""Contains asynchronous HTTP PUT request with automatic retry
logic."""
...
async def put_async(
    ...
    **kwargs: Any,
    ...
    This function performs an HTTP PUT request with a configured retry
    ...
    Example:
        ```pycon
        >>> import asyncio
        >>> from aresilient import put_async
        >>> from aresilient.core import ClientConfig
        >>> config = ClientConfig(max_retries=5)
        >>> async def example():
        ...     response = await put_async("https://api.example.com/data", config=config)
