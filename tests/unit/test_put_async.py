...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_successful_put(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test successful PUT request with custom client."""
    response = await put_with_automatic_retry_async(TEST_URL, client=mock_client)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_successful_put_request_with_default_client(
    mock_response: httpx.Response, mock_asleep: Mock
) -> None:
    """Test successful PUT request on first attempt."""
    with patch("httpx.AsyncClient.put", new_callable=AsyncMock, return_value=mock_response):
        response = await put_with_automatic_retry_async(TEST_URL)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_put_with_json_payload(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test PUT request with JSON data."""
    response = await put_with_automatic_retry_async(
        TEST_URL, json={"key": "value"}, client=mock_client
    )
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_retry_on_500_status(
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_retry_on_503_status(
    mock_response: httpx.Response, mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_max_retries_exceeded(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_non_retryable_status_code(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_exponential_backoff(
    mock_response: httpx.Response, mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test exponential backoff timing."""
    mock_response_fail = Mock(spec=httpx.Response, status_code=503)
    mock_client.put.side_effect = [mock_response_fail, mock_response_fail, mock_response]

    await put_with_automatic_retry_async(TEST_URL, client=mock_client, backoff_factor=2.0)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_timeout_exception(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test handling of timeout exception."""
    mock_client.put.side_effect = httpx.TimeoutException("Request timeout")
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_timeout_exception_with_retries(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_request_error(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_request_error_with_retries(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test handling of general request errors."""
    mock_client.put.side_effect = httpx.RequestError("Connection failed")
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_negative_max_retries() -> None:
    """Test that negative max_retries raises ValueError."""
    with pytest.raises(ValueError, match=r"max_retries must be >= 0"):
        await put_with_automatic_retry_async(TEST_URL, max_retries=-1)


@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_negative_backoff_factor() -> None:
    """Test that negative backoff_factor raises ValueError."""
    with pytest.raises(ValueError, match=r"backoff_factor must be >= 0"):
        await put_with_automatic_retry_async(TEST_URL, backoff_factor=-1.0)


@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_zero_max_retries(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test with zero retries - should only try once."""
    mock_client.put.return_value = Mock(spec=httpx.Response, status_code=503)

    with pytest.raises(
        HttpRequestError,
        match=rf"PUT request to {TEST_URL} failed with status 503 after 1 attempts",
    ):
        await put_with_automatic_retry_async(TEST_URL, client=mock_client, max_retries=0)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_custom_status_forcelist(
    mock_response: httpx.Response, mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
async def test_put_with_automatic_retry_async_default_retry_status_codes(
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_client_close_when_owns_client(
    mock_client: httpx.AsyncClient,
) -> None:
    """Test that client is closed when created internally."""
    with patch("httpx.AsyncClient", return_value=mock_client):
        await put_with_automatic_retry_async(TEST_URL)
    mock_client.aclose.assert_called_once()


@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_client_not_closed_when_provided(
    mock_client: httpx.AsyncClient,
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_custom_timeout(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test custom timeout parameter."""
    with patch("httpx.AsyncClient") as mock_client_class:
        mock_client_class.return_value = mock_client
        await put_with_automatic_retry_async(TEST_URL, timeout=30.0)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_all_retries_with_429(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    ...
    with pytest.raises(HttpRequestError) as exc_info:
        await put_with_automatic_retry_async(TEST_URL, client=mock_client, max_retries=1)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_with_httpx_timeout_object(
    mock_response: httpx.Response, mock_asleep: Mock
) -> None:
    """Test PUT request with httpx.Timeout object."""
    ...
    with patch("httpx.AsyncClient") as mock_client_class:
        ...
        response = await put_with_automatic_retry_async(TEST_URL, timeout=timeout_config)
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_recovery_after_multiple_failures(
    mock_response: httpx.Response, mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    ...
    response = await put_with_automatic_retry_async(TEST_URL, client=mock_client, max_retries=5)
...
async def test_put_with_automatic_retry_async_successful_2xx_status_codes(
...
async def test_put_with_automatic_retry_async_successful_3xx_status_codes(
    mock_asleep: Mock, mock_client: httpx.AsyncClient, status_code: int
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_with_headers(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_error_message_includes_url(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_client_close_on_exception(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_mixed_error_and_status_failures(
    mock_response: httpx.Response, mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_network_error(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_read_error(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test that ReadError is retried appropriately."""
    mock_client.put.side_effect = httpx.ReadError("Connection broken")
    with (
        pytest.raises(
            HttpRequestError,
            match=r"PUT request to https://api.example.com/resource failed after 4 attempts",
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_write_error(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test that WriteError is retried appropriately."""
    mock_client.put.side_effect = httpx.WriteError("Write failed")
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_connect_timeout(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_read_timeout(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_pool_timeout(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test that PoolTimeout is retried appropriately."""
    mock_client.put.side_effect = httpx.PoolTimeout("Connection pool exhausted")
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_proxy_error(
    mock_client: httpx.AsyncClient, mock_asleep: Mock
) -> None:
    """Test that ProxyError is retried appropriately."""
    mock_client.put.side_effect = httpx.ProxyError("Proxy connection failed")
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_validation_negative_max_retries(
    mock_client: httpx.AsyncClient,
) -> None:
    """Test that negative max_retries raises ValueError."""
    with pytest.raises(ValueError, match=r"max_retries must be >= 0"):
...
@pytest.mark.asyncio
async def test_put_with_automatic_retry_async_validation_negative_backoff_factor(
    mock_client: httpx.AsyncClient,
) -> None:
    """Test that negative backoff_factor raises ValueError."""
    with pytest.raises(ValueError, match=r"backoff_factor must be >= 0"):
