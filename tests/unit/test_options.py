...
def mock_client(mock_response: httpx.Response) -> httpx.Client:
    return Mock(spec=httpx.Client, options=Mock(return_value=mock_response))
...
def test_options_with_automatic_retry_successful_options(
...
def test_options_with_automatic_retry_successful_options_request_with_default_client(
    mock_response: httpx.Response, mock_sleep: Mock
) -> None:
    """Test successful OPTIONS request on first attempt."""
    with patch("httpx.Client.options", return_value=mock_response):
        response = options_with_automatic_retry(TEST_URL)

    assert response.status_code == 200
    mock_sleep.assert_not_called()


def test_options_with_automatic_retry_retry_on_500_status(
...
def test_options_with_automatic_retry_retry_on_503_status(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_max_retries_exceeded(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with pytest.raises(HttpRequestError) as exc_info:
        options_with_automatic_retry(TEST_URL, client=mock_client, max_retries=2)
...
def test_options_with_automatic_retry_non_retryable_status_code(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with pytest.raises(
        ...
        options_with_automatic_retry(TEST_URL, client=mock_client)

    mock_sleep.assert_not_called()


def test_options_with_automatic_retry_exponential_backoff(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    options_with_automatic_retry(TEST_URL, client=mock_client, backoff_factor=2.0)

    # Should have slept twice (after 1st and 2nd failures)
    assert mock_sleep.call_args_list == [call(2.0), call(4.0)]


def test_options_with_automatic_retry_timeout_exception(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with pytest.raises(
        ...
        options_with_automatic_retry(TEST_URL, client=mock_client, max_retries=0)

    mock_sleep.assert_not_called()


def test_options_with_automatic_retry_timeout_exception_with_retries(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_request_error(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_request_error_with_retries(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    """Test handling of general request errors."""
    mock_client.options.side_effect = httpx.RequestError("Connection failed")
...
def test_options_with_automatic_retry_negative_max_retries() -> None:
    """Test that negative max_retries raises ValueError."""
    with pytest.raises(ValueError, match=r"max_retries must be >= 0"):
        options_with_automatic_retry(TEST_URL, max_retries=-1)


def test_options_with_automatic_retry_negative_backoff_factor() -> None:
    """Test that negative backoff_factor raises ValueError."""
    with pytest.raises(ValueError, match=r"backoff_factor must be >= 0"):
        options_with_automatic_retry(TEST_URL, backoff_factor=-1.0)


def test_options_with_automatic_retry_negative_jitter_factor() -> None:
    """Test that negative jitter_factor raises ValueError."""
    with pytest.raises(ValueError, match=r"jitter_factor must be >= 0"):
        options_with_automatic_retry(TEST_URL, jitter_factor=-0.1)


def test_options_with_automatic_retry_with_jitter_factor(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_zero_jitter_factor(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_zero_max_retries(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_custom_status_forcelist(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    response = options_with_automatic_retry(TEST_URL, client=mock_client, status_forcelist=(404,))
...
def test_options_with_automatic_retry_client_not_closed_when_provided(
    mock_client: httpx.Client,
) -> None:
...
def test_options_with_automatic_retry_custom_timeout(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    """Test custom timeout parameter."""
    with patch("httpx.Client") as mock_client_class:
        mock_client_class.return_value = mock_client
        options_with_automatic_retry(TEST_URL, timeout=30.0)

    mock_client_class.assert_called_once_with(timeout=30.0)
    mock_sleep.assert_not_called()


def test_options_with_automatic_retry_all_retries_with_429(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with pytest.raises(HttpRequestError) as exc_info:
        options_with_automatic_retry(TEST_URL, client=mock_client, max_retries=1)
...
def test_options_with_automatic_retry_with_httpx_timeout_object(
    mock_response: httpx.Response, mock_sleep: Mock
) -> None:
    ...
    with patch("httpx.Client") as mock_client_class:
        ...
        mock_client_instance.options = Mock(return_value=mock_response)
        mock_client_instance.close = Mock()
        mock_client_class.return_value = mock_client_instance
        response = options_with_automatic_retry(TEST_URL, timeout=timeout_config)
...
def test_options_with_automatic_retry_recovery_after_multiple_failures(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    response = options_with_automatic_retry(TEST_URL, client=mock_client, max_retries=5)
...
def test_options_with_automatic_retry_successful_2xx_status_codes(
...
def test_options_with_automatic_retry_successful_3xx_status_codes(
    mock_sleep: Mock, mock_client: httpx.Client, status_code: int
) -> None:
...
def test_options_with_automatic_retry_with_headers(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_error_message_includes_url(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_client_close_on_exception(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with (
        ...
        options_with_automatic_retry(TEST_URL, max_retries=0)

    mock_client.close.assert_called_once()
    mock_sleep.assert_not_called()


def test_options_with_automatic_retry_mixed_error_and_status_failures(
    mock_response: httpx.Response, mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_network_error(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
    ...
    with pytest.raises(
        ...
        options_with_automatic_retry(TEST_URL, client=mock_client, max_retries=3)
...
def test_options_with_automatic_retry_connect_timeout(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_read_timeout(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_pool_timeout(
    mock_client: httpx.Client, mock_sleep: Mock
) -> None:
...
def test_options_with_automatic_retry_validation_negative_max_retries(
    mock_client: httpx.Client,
) -> None:
    """Test that negative max_retries raises ValueError."""
    with pytest.raises(ValueError, match=r"max_retries must be >= 0"):
...
def test_options_with_automatic_retry_validation_negative_backoff_factor(
    mock_client: httpx.Client,
) -> None:
    """Test that negative backoff_factor raises ValueError."""
    with pytest.raises(ValueError, match=r"backoff_factor must be >= 0"):
        options_with_automatic_retry(TEST_URL, client=mock_client, backoff_factor=-0.5)
