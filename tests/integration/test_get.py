...
def test_get_redirect_chain() -> None:
    ...
    with httpx.Client(follow_redirects=True) as client:
        response = get(url=f"{HTTPBIN_URL}/redirect/3", client=client)
...
def test_get_large_response() -> None:
    """Test GET request with large response body."""
    # Request a large amount of bytes (10KB)
    with httpx.Client() as client:
        response = get(url=f"{HTTPBIN_URL}/bytes/10240", client=client)

    assert response.status_code == 200
    assert len(response.content) == 10240


def test_get_with_custom_headers() -> None:
